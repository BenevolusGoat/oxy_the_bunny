---@param ent Entity?
function OxyTheBunny:IsValidEnemyTarget(ent)
	return ent
		and ent:ToNPC()
...
function OxyTheBunny:GetClosestEnemy(pos, range, filter)
	---@type EntityNPC | nil
	local closestEnemy
	...
	for _, ent in ipairs(entities) do
		local npc = ent:ToNPC()
		if npc and (not filter and OxyTheBunny:IsValidEnemyTarget(npc) or filter and filter(npc)) then
			---@cast npc EntityNPC
...
---@return EntityNPC | nil
function OxyTheBunny:GetClosestEnemyInView(pos, range, dir, fov, occludeObstacles, occludeWalls)
	---@type EntityNPC | nil
	local closestEnemy
	local closestDistance
	local entities = {}
	...
	for _, ent in pairs(entities) do
		local npc = ent:ToNPC()
		if not OxyTheBunny:IsValidEnemyTarget(npc) then goto continue end
		---@cast npc EntityNPC
		...
		local noOcclude = OxyTheBunny.Room():CheckLine(pos, ent.Position, 3, threshold, occludeWalls)
...
function OxyTheBunny:GetClosestEntity(pos, range, partition)
	---@type Entity
	local closestEnt
	local closestDistance
