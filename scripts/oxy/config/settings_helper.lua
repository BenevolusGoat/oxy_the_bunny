...
OxyTheBunny.SettingsHelper = SettingsHelper

OxyTheBunny.SettingTypes = {
	Choice = 0,
	Keybind = 1,
	Boolean = 2,
}
...
---@param value any
---@function
function OxyTheBunny.SaveSetting(settingKey, value)
	local game_save = OxyTheBunny:GameSave()
...
function OxyTheBunny.GetSettingStr(settingKey)
	local settingValue = OxyTheBunny.GetSetting(settingKey)
...
function SettingsHelper.AddChoiceSetting(category, settingName, settingDescription, possibleValues, defaultValue,
	...
	table.insert(group, {
		...
		Condition = condition,
		Type = OxyTheBunny.SettingTypes.Choice,
	})
end
...
function SettingsHelper.AddBooleanSetting(category, settingName, settingDescription, defaultValue, condition)
	...
	table.insert(group, {
		...
		Default = defaultValue,
		Condition = condition,
		Type = OxyTheBunny.SettingTypes.Boolean,
	})
end
...
function SettingsHelper.AddKeybindSetting(category, settingName, settingDescription, defaultKey, condition)
	...
	table.insert(group, {
		...
		Default = defaultKey,
		Condition = condition,
		Type = OxyTheBunny.SettingTypes.Keybind,
	})
end
