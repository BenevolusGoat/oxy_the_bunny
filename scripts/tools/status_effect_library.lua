...
local function InitMod()
	...
	StatusEffectLibrary.AddedCallbacks = game:GetFrameCount() == 0 and CACHED_MOD_CALLBACKS or
	StatusEffectLibrary.AddedCallbacks
...
local function InitFunctions()
	...
	function StatusEffectLibrary.Utils.GetLastParent(ent)
		...
		while StatusEffectLibrary.Utils.IsInParentChildChain(currentEnt) and not parents[GetPtrHash(currentEnt)] and not parents[GetPtrHash(currentEnt.Parent)] do
			StatusEffectLibrary.Utils.DebugLog("N/A", "Jumped to parent", ent.Parent.Type, ent.Parent.Variant, "from",
				ent.Type, ent.Variant)
	...
	StatusEffectLibrary.CallbackHandlers = {
		...
		[StatusEffectLibrary.Callbacks.ID.PRE_ADD_ENTITY_STATUS_EFFECT] = function(callbacks, entity, statusEffect,
																				   customData)
			for i = 1, #callbacks do
				local limits = callbacks[i].Args
		...
		[StatusEffectLibrary.Callbacks.ID.PRE_REMOVE_ENTITY_STATUS_EFFECT] = function(callbacks, entity, statusEffect,
																					  statusEffectData)
			for i = 1, #callbacks do
				local limits = callbacks[i].Args
				local shouldFire = not limits[1] or StatusEffectLibrary.Utils.HasBitFlags(statusEffect, limits[1])
		...
		[StatusEffectLibrary.Callbacks.ID.POST_REMOVE_ENTITY_STATUS_EFFECT] = function(callbacks, entity, statusEffect,
																					   statusEffectData)
			for i = 1, #callbacks do
				local limits = callbacks[i].Args
				local shouldFire = not limits[1] or StatusEffectLibrary.Utils.HasBitFlags(statusEffect, limits[1])
	...
	function StatusEffectLibrary:AddStatusEffect(ent, statusFlag, duration, source, color, customData, calledInLoop)
		...
		if not statusEffects then
			...
			StatusEffectLibrary.Utils.DebugLog("N/A", "Initialized general status effect data for", ent.Type, ent
			.Variant)
		...
		if not statusEffectData then
			...
			StatusEffectLibrary.Utils.DebugLog(identifier, "Initialized status effect data with duration of", duration,
				"for", ent.Type, ent.Variant)
		else
			statusEffectData.Countdown = floor(duration)
			StatusEffectLibrary.Utils.DebugLog(identifier, "Status effect already present. Resetting duration to",
				duration, "for", ent.Type, ent.Variant)
			return true
		end
		...
		then
			...
			do
				StatusEffectLibrary.Utils.DebugLog(identifier, "Jumped to child", currentEnt.Child:ToNPC().Type,
					currentEnt.Child:ToNPC().Variant, "from", currentEnt.Type, currentEnt.Variant)
				children[entHash] = true
				applyNonLoopingStatus(currentEnt, statusFlag, duration, source, color,
					StatusEffectLibrary.Utils.DeepCopy(customData), isLooped)
			...
			if currentEnt.Parent
				...
				and not children[entHash]
			then
				StatusEffectLibrary.Utils.DebugLog(identifier, "Applied to end of chain", currentEnt.Type,
					currentEnt.Variant)
				applyNonLoopingStatus(currentEnt, statusFlag, duration, source, color,
					StatusEffectLibrary.Utils.DeepCopy(customData), isLooped)
			end
		...
		then
			...
			StatusEffectLibrary.Utils.DebugLog(identifier, "Icon added. Max number of icons is",
				statusEffects.NumIconsActive)
		end
	...
	function StatusEffectLibrary.OnStatusEffectRender(_, ent, offset)
		...
		local result = StatusEffectLibrary.Callbacks.FireCallback(
		StatusEffectLibrary.Callbacks.ID.PRE_RENDER_STATUS_EFFECTS, ent)
		...
		for _, statusEffectData in pairs(statusEffects.StatusEffectData) do
			if statusEffectData.Icon then
				local dynamicOffset = Vector((-8 * (statusEffects.NumIconsActive - 1)) + (16 * iconIndex), 0) +
				vanillaOffset
	...
	AddPriorityCallback(ModCallbacks.MC_PRE_NPC_UPDATE, CallbackPriority.IMPORTANT,
		StatusEffectLibrary.OnStatusEffectUpdate)
