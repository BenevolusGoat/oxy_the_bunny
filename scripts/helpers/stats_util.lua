...
function OxyTheBunny:GetWeaponOwnerDamageMult(owner)
	local damageMult = 1
	...
	if variantToDamage[owner.Variant] then
		...
		if owner.Variant == FamiliarVariant.BLOOD_BABY then
			local subtypeToDamage = {
				[OxyTheBunny.BloodClotSubtype.BLACK] = 0.43,
				[OxyTheBunny.BloodClotSubtype.ETERNAL] = 0.52
			}
...
---@param rng RNG
function OxyTheBunny:DoesLuckChanceTrigger(baseChance, maxChance, luckValue, currentLuck, rng)
...
---@return string
function OxyTheBunny:TearScaleToSizeAnim(tear)
...
function OxyTheBunny:GetBombRadius(bomb)
...
---@param tear EntityTear
function OxyTheBunny:TearDamageToScale(tear)
	local baseScale = 1.0
