---@param ent1 Entity
---@param ent2 Entity
function OxyTheBunny:IsSameEntity(ent1, ent2)
...
function OxyTheBunny:IsSpriteAnimActive(sprite, anims, useOverlay)
	for i = 1, #anims do
...
function OxyTheBunny:IsSplitTear(tear)
	local isSplit = false
	...
	for _, tears in pairs(Isaac.FindInRadius(tear.Position, 10, EntityPartition.TEAR)) do
		...
		if tear.InitSeed ~= mainTear.InitSeed
			...
			and OxyTheBunny:TryGetPlayer(tear)
		then
			isSplit = true
		end
	end
...
---@param c1 Color
---@param c2 Color
function OxyTheBunny:AreColorsDifferent(c1, c2)
...
function OxyTheBunny:IsBrimLaser(laser)
	local brimVariants = OxyTheBunny:Set({
...
function OxyTheBunny:CopySprite(sprite)
...
---@param color Color
function OxyTheBunny:CopyColor(color)
	return Color(color.R, color.G, color.B, color.A, color.RO, color.GO, color.BO)
end

---@param func function
function OxyTheBunny:DelayOneFrame(func)
	Isaac.CreateTimer(func, 1, 1, true)
end

---@param dir Direction
function OxyTheBunny:DirectionToString(dir)
	local directions = {
...
---@param ... any
---@return T
function OxyTheBunny:ProcessFuncOrValue(val, ...)
...
---@param var integer
---@param sub integer
function OxyTheBunny:CheckTypeVarSub(ent, type, var, sub)
...
function OxyTheBunny:SpawnNotifyEffect(pos, subType)
...
function OxyTheBunny:RoomCheckLineEx(pos1, pos2, lineCheckMode, gridPath, ignoreWalls, ignoreCrushable)
	local isClearPath, shittyPosIfObstructed = OxyTheBunny.Room():CheckLine(pos1, pos2, lineCheckMode, gridPath,
		ignoreWalls,
		ignoreCrushable)
	...
	if not isClearPath then
		...
		for _ = 1, 10 do
			local gridEnt = OxyTheBunny.Room():GetGridEntityFromPos(findGridPos)
...
---@param pos Vector
function OxyTheBunny:GetClosestGridEnt(pos)
	...
	for row = 1, 3 do
		...
		for column = 1, 3 do
			local offset = 0
			local roomShape = OxyTheBunny.Room():GetRoomShape()
...
function OxyTheBunny:GetRandomPlaydoughColor()
	local dC = Color.Default
	local color = playdoughColor[OxyTheBunny:RandomNum(9)]
...
function OxyTheBunny:GetClosestDoorSlotPos(pos)
	local closestDoor
	local closestDistance

	for doorSlot = 0, DoorSlot.NUM_DOOR_SLOTS do
		local door = OxyTheBunny.Room():GetDoor(doorSlot)
		if door ~= nil and door:IsOpen() then
			local doorPos = OxyTheBunny.Room():GetDoorSlotPosition(doorSlot)
...
---@param str string
function OxyTheBunny:ToEnum(str)
	return string.gsub(str, " ", "_"):upper()
end

function OxyTheBunny:ShouldUpdateSprite()
	return not OxyTheBunny.Game:IsPaused() and Isaac.GetFrameCount() % 2 == 0
end

function OxyTheBunny:TryStartAmbush()
	local room = OxyTheBunny.Room():GetType()
...
---@param tear EntityTear
function OxyTheBunny:TryChangeTearToBloodVariant(tear)
...
local inverseTearTable = OxyTheBunny:Invert(bloodTearTable)

function OxyTheBunny:IsBloodTear(tear)
	return inverseTearTable[tear.Variant] ~= nil
end

---@param tear EntityTear
---@param player EntityPlayer
function OxyTheBunny:ShouldUpdateLudo(tear, player)
	return math.floor(tear.FrameCount / player.MaxFireDelay) ~= math.floor((tear.FrameCount - 1) / player.MaxFireDelay)
end

function OxyTheBunny:IsDeadEnemy(ent)
	return ent:IsActiveEnemy(true) or ent:ToNPC() and ent:ToNPC().CanShutDoors
end

---@return string
function OxyTheBunny:GetHealthPath()
