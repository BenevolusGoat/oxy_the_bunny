OxyTheBunny.FamliiarDirections = {
	"Side",
	"Up",
	"Side",
	"Down"
...
function OxyTheBunny:ShouldFamiliarAutoAim(player)
...
function OxyTheBunny:GetFamiliarAimVector(familiar)
	local player = familiar.Player
	local aimVector = OxyTheBunny:DirectionToVector(player:GetFireDirection())
	...
	then
		local targetAimVector = OxyTheBunny:TryGetMarkedTargetAimVector(player)

		if targetAimVector then
			aimVector = targetAimVector
			isAutoAim = true
		end
	elseif OxyTheBunny:ShouldFamiliarAutoAim(player) then
		local closestEnemy = OxyTheBunny.Character.BLUEBABY.CREEP.SENTIENT:FindClosestEnemy(familiar,
